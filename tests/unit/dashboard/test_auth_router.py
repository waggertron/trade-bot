...
class TestLogin:
    async def test_login_returns_tokens(self, client: AsyncClient):
        await client.post("/api/auth/register", json={
            "email": "bob@example.com", "password": "Secret99!", "name": "Bob",
        })
        resp = await client.post("/api/auth/login", json={
            "email": "bob@example.com", "password": "Secret99!",
        })
...
class TestRefresh:
    async def test_refresh_returns_new_access_token(self, client: AsyncClient):
        reg = await client.post("/api/auth/register", json={
            "email": "carol@example.com", "password": "Refresh1!", "name": "Carol",
        })
...
class TestMe:
    async def test_me_returns_current_user(self, client: AsyncClient):
        reg = await client.post("/api/auth/register", json={
            "email": "dave@example.com", "password": "MyPass1!", "name": "Dave",
        })
...
class TestUpdateMe:
    async def test_update_name(self, client: AsyncClient):
        reg = await client.post("/api/auth/register", json={
            "email": "eve@example.com", "password": "Update1!", "name": "Eve",
        })
        token = reg.json()["access_token"]
