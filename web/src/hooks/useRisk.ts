...
import { getRiskStatus, getDrawdownStatus, getCircuitBreaker, getCurrentRegime, getRiskPresets } from "@/lib/api";
...
export function useDrawdownStatus() {
  return useQuery({ queryKey: ["drawdown-status"], queryFn: getDrawdownStatus, refetchInterval: 10000 });
}

export function useCircuitBreaker() {
  return useQuery({ queryKey: ["circuit-breaker"], queryFn: getCircuitBreaker, refetchInterval: 10000 });
}
