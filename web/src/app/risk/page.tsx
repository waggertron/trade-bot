...
import { useRiskStatus, useDrawdownStatus, useCircuitBreaker, useRegime, useRiskPresets } from "@/hooks/useRisk";
import { updateRiskSettings, applyRiskPreset, resetCircuitBreaker } from "@/lib/api";
import ChartContainer from "@/components/shared/ChartContainer";
import StatCard from "@/components/shared/StatCard";
import { StatCardSkeleton } from "@/components/shared/LoadingSkeleton";
import DataTable from "@/components/shared/DataTable";
import { formatPercent, cn } from "@/lib/formatters";
import { ShieldAlert, AlertTriangle, Activity, RotateCcw } from "lucide-react";
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs">
        ...
        <span className="text-foreground">{value.toFixed(1)} / {max}</span>
      </div>
...
export default function RiskPage() {
  ...
  const { data: presets } = useRiskPresets();
    ...
    <div className="space-y-6">
        ...
        {riskLoading ? (
          ...
          <>
            <StatCard
              ...
              value={((regime as Record<string, unknown>)?.regime as string || "medium").toUpperCase()}
              icon={Activity}
            />
            <StatCard
        ...
        <ChartContainer title="Circuit Breaker" subtitle="Emergency stop status">
          <div className="space-y-4">
            <div className={cn(
              ...
              cb?.tripped ? "bg-loss/10 border border-loss/30" : "bg-profit/10 border border-profit/30"
            )}>
              <div className={cn(
              ...
              <div>
                ...
                <p className="text-xs text-muted">
                  {cb?.tripped ? "Trading halted — drawdown exceeded threshold" : "All systems operational"}
                </p>
              </div>
            </div>
            {cb?.tripped === true && (
      ...
      >
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <SliderField label="Max Position %" value={getVal("max_position_pct", 2)} min={0.5} max={20} step={0.5} unit="%" onChange={(v) => setFormValues({ ...formValues, max_position_pct: v })} />
            <SliderField label="Max Sector Exposure %" value={getVal("max_sector_exposure_pct", 20)} min={5} max={50} step={5} unit="%" onChange={(v) => setFormValues({ ...formValues, max_sector_exposure_pct: v })} />
            <SliderField label="Daily Loss Limit %" value={getVal("daily_loss_limit_pct", 3)} min={1} max={15} step={0.5} unit="%" onChange={(v) => setFormValues({ ...formValues, daily_loss_limit_pct: v })} />
            <SliderField label="Weekly Drawdown Limit %" value={getVal("weekly_drawdown_limit_pct", 5)} min={1} max={20} step={1} unit="%" onChange={(v) => setFormValues({ ...formValues, weekly_drawdown_limit_pct: v })} />
            <SliderField label="Max Open Positions" value={getVal("max_open_positions", 10)} min={1} max={30} step={1} unit="" onChange={(v) => setFormValues({ ...formValues, max_open_positions: v })} />
            <SliderField label="Stop Loss %" value={getVal("stop_loss_pct", 5)} min={1} max={20} step={0.5} unit="%" onChange={(v) => setFormValues({ ...formValues, stop_loss_pct: v })} />
            <SliderField label="Trailing Stop %" value={getVal("trailing_stop_pct", 3)} min={1} max={15} step={0.5} unit="%" onChange={(v) => setFormValues({ ...formValues, trailing_stop_pct: v })} />
            <SliderField label="Max Correlation" value={getVal("max_correlation", 0.7)} min={0.1} max={1} step={0.05} unit="" onChange={(v) => setFormValues({ ...formValues, max_correlation: v })} />
          </div>
        </div>
      </ChartContainer>
    </div>
