...
import { getNewsArticles, getNewsStatus, getSentimentAggregate } from "@/lib/api";
import { cn, formatTimeAgo } from "@/lib/formatters";

function SentimentBadge({ score }: { score: number }) {
  if (score > 0.2)
    return (
      <span className="rounded-full bg-profit/20 px-2 py-0.5 text-xs text-profit">Bullish</span>
    );
  if (score < -0.2)
    return <span className="rounded-full bg-loss/20 px-2 py-0.5 text-xs text-loss">Bearish</span>;
...
export default function NewsPage() {
  const { data: articles, isLoading } = useQuery({
    ...
    queryFn: () => getNewsArticles({ limit: 50 }),
            ...
            {isLoading ? (
              ...
              <div className="py-16 text-center text-muted">
                ...
                <p className="mt-1 text-xs">
                  News providers will populate articles when configured
                </p>
              </div>
            ) : (
              <div className="max-h-[600px] space-y-2 overflow-y-auto">
                {articleList.map((article, i) => (
                  <div
                    key={i}
                    className="rounded-lg border border-border/50 p-3 hover:bg-card-hover/50"
                  >
          ...
          <ChartContainer title="Provider Status">
            <div className="space-y-2">
              {(newsStatus as Record<string, unknown>)?.providers ? (
                (
                  (newsStatus as Record<string, unknown>).providers as Record<string, unknown>[]
                ).map((p, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between rounded-lg bg-background p-3"
